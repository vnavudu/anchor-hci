...
const CalmSession = () => {
  ...
  const cadenceSeconds = exercise?.cadenceSeconds
  const animationCadence = cadenceSeconds ?? Math.max(MIN_STEP_SECONDS, 8)
  ...
  return (
    <section className="flex w-full flex-col items-center gap-10 text-center">
      ...
      {exercise && phase !== 'choose' && (
        <div className="w-full max-w-3xl animate-fade-in-up rounded-[3rem] border border-white/70 bg-white/95 p-8 text-left shadow-soft backdrop-blur-sm">
          <div className="flex flex-col gap-3 border-b border-anchor-muted/10 pb-6 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-4">
              <div className="flex h-16 w-16 items-center justify-center rounded-3xl bg-[#EAF1F8] text-3xl">
                {exercise.icon}
              </div>
              <div>
            ...
            <div className="flex flex-col items-end gap-3 text-right">
              <span className="rounded-2xl bg-[#D7E4F3] px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-anchor-primary">
                {isGuidedMode ? 'guided mode' : 'manual mode'}
              </span>
              <button
                type="button"
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {['guided', 'manual'].map((value) => (
              <button
                ...
                className={`flex-1 rounded-[2rem] border px-4 py-3 text-xs font-semibold uppercase tracking-[0.3em] transition-all duration-200 ${
                  mode === value
                    ? 'border-anchor-primary bg-anchor-primary/10 text-anchor-primary shadow-soft'
                    : 'border-anchor-muted/40 text-anchor-muted hover:bg-[#EEF2F8]'
                }`}
              >
                {value} mode
              </button>
          ...
          {(phase === 'prepare' || phase === 'manual') && (
            <div className="mt-6 space-y-6">
              ...
              <div className="grid gap-6 lg:grid-cols-2">
                <div className="flex flex-col items-center justify-center gap-4 rounded-[2.5rem] border border-anchor-muted/20 bg-[#F7FAFF] p-6 shadow-soft">
                  <ExerciseAnimation
                  ...
                  <p className="text-center text-sm text-anchor-muted">
                    {isGuidedMode
                      ? 'Preview how the guided pace will feel. Tap start when you are ready.'
                      : 'Follow these visuals while you move through the steps at your own pace.'}
                  </p>
                </div>
                <div className="rounded-[2.5rem] border border-anchor-muted/20 bg-white/90 p-6 shadow-soft">
                  <p className="text-xs font-semibold uppercase tracking-[0.3em] text-anchor-muted">
                    Full instructions
                  </p>
                  <InstructionList steps={exercise.steps} />
                  {exercise.tip && (
                    <p className="mt-4 text-xs text-anchor-muted">
                      Why it helps: {exercise.tip}
                    </p>
                  )}
                </div>
              </div>
              {isGuidedMode ? (
                <div className="flex flex-wrap gap-4">
                  <button
                    ...
                    className="flex-1 rounded-[2rem] bg-anchor-primary px-6 py-4 text-sm font-semibold uppercase tracking-[0.3em] text-white shadow-soft transition-transform duration-200 hover:-translate-y-0.5 hover:bg-[#003B8E]"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 rounded-[2rem] border border-anchor-muted/40 bg-white px-6 py-4 text-sm font-semibold uppercase tracking-[0.3em] text-anchor-muted transition-transform duration-200 hover:-translate-y-0.5 hover:bg-[#EEF2F8]"
                  >
          ...
          {phase === 'guide' && (
            <div className="mt-6 space-y-6">
              <div className="flex flex-col gap-3 rounded-[2.5rem] border border-anchor-muted/20 bg-[#F7FAFF] p-6 text-anchor-deep shadow-soft sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.3em] text-anchor-muted">
                    Guided pacing
                  </p>
              ...
              <div className="grid gap-6 lg:grid-cols-[1fr_1fr]">
                <div className="flex flex-col items-center justify-center rounded-[2.5rem] border border-anchor-muted/20 bg-white/90 p-6 shadow-soft">
                  <ExerciseAnimation
                    type={exercise.animation}
                    duration={(animationCadence || 8) + 2}
                  />
                ...
                <div className="rounded-[2.5rem] border border-anchor-muted/20 bg-[#F7FAFF] p-6 shadow-soft">
                  ...
                  <div className="mt-6 flex flex-wrap gap-4">
                    <button
                      type="button"
                      onClick={handleManualComplete}
                      className="flex-1 rounded-[2rem] border border-anchor-muted/30 px-6 py-3 text-xs font-semibold uppercase tracking-[0.3em] text-anchor-muted transition-colors duration-200 hover:bg-[#EEF2F8]"
                    >
                      finish early
                    </button>
                    <button
          ...
          {phase === 'reflect' && (
            <div className="mt-6 space-y-5">
              <div className="rounded-[2.5rem] border border-anchor-muted/20 bg-[#F7FAFF] p-6 text-anchor-deep shadow-soft">
                <p className="text-sm font-semibold uppercase tracking-[0.3em] text-anchor-muted">
                  Nice work
                </p>
              ...
              <div className="flex flex-wrap gap-4">
                <button
                  ...
                  className="flex-1 rounded-[2rem] border border-anchor-primary/30 px-6 py-3 text-xs font-semibold uppercase tracking-[0.3em] text-anchor-primary transition-colors duration-200 hover:bg-[#EAF1F8]"
                >
                  replay instructions
                </button>
                <button
